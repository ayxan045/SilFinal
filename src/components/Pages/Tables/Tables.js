...
const Tables = (props) => {
    ...
    const setEditingObject = async (i) => {
        setEditing(i);
        await admin.get(`/tables/${i}`).then((res) => {
            let data = res.data
        ...
        <Row gutter={[10, 10]}>
            <Col xs={24}>
                <div className="border animated fadeInDown p-2 mt-0 bg-white">
                    ...
                    <span className="f-20 bold">Masalar</span>
                </div>
            </Col>
            <Col lg={12} xs={24}>
                <Table
            ...
            <Col lg={12} xs={24}>
                <Card title={t("addTo")} className={"animated fadeInRight"}>
                    <Form layout="vertical" onFinish={savePosition} form={form}>
                        <p className="mb-5">Masanın adı</p>
...
export default connect(null, { notify })(Tables);
